...
class LoginController extends Controller
{
    ...
    protected function otp_verify(Request $request)
    {
        $userBll = new User();
        $userBll->loginByOtp($request);
        if (request()->query("return") != null) {
            return redirect()->to(url(request()->query("return")));
        }
        return redirect()->intended('home');
    }
    protected function otp_send(Request $request)
    {
        if (!$request->ajax()) {
        ...
        $sms = new SMS();
        $bool = $sms->send($request->input("otp_mobile"));
       // dd($sms);
        return response()->json([
            "status" => $bool,
            "data" => $bool ? $sms->getResponse() : $sms->getError()
        ]);
    }
    ...
    protected function checkLogin(Request $request)
    {
        ...
        $user = WebsiteUser::all()->whereEmail($request->input("login_email"))->first();
        if ($user == null) {
        ...
        if (auth()->guard('web')->attempt(['email' => $request->login_email, 'password' => $request->login_password], $remember_me)) {
