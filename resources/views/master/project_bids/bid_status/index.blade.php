...
@extends('master.layout.index' ,['title' => _i('Bid Statuses')])

@section('content')
    <!--breadcrumbs start -->
    <ul class="breadcrumb">
        <li><a href="{{route('MasterHome')}}"><i class="fa fa-home"></i> {{_i('Home')}}</a></li>
        <li class="active">{{_i('Bid Statuses')}}</li>
    </ul>
    <!--breadcrumbs end -->
            ...
            <section class="panel">
                <header class="panel-heading">
                    <a href="#" class='btn btn-shadow btn-primary mb-2' data-toggle='modal' data-target='#add-Modal'>
                        <i class="fa fa-plus"> </i> {{ _i('Create new bid status') }}
                    </a>
                </header>
    ...
    <!-- page end-->
@include('master.project_bids.bid_status.create')
@include('master.project_bids.bid_status.edit')
@endsection
...
@push('js')
    <script>
        var table = $('#datatable').DataTable({
            processing: true,
            serverSide: true,
            ajax: '{{ route('master.bid_status.index')}}',
            columns: [
                {data: 'id'},
                {data: 'title'},
                {data: 'created_at'},
