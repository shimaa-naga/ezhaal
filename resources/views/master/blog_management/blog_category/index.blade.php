@extends('master.layout.index' ,['title' => _i('Blog Categories')])

@section('content')
    <!--breadcrumbs start -->
    <div class="page-header">
        <h4 class="page-title">{{ _i('Blog Categories') }}</h4>
        <ol class="breadcrumb">
                ...
                <div class="card-header">
                    <div class="card-title">
                        <a href="#" class='btn btn-shadow btn-primary mb-2 text-white' data-toggle='modal' data-target='#add-Modal'>
                            <i class="fa fa-plus"> </i> {{ _i('Create new Blog Category') }}
                        </a>
                    </div>
                </div>
                <div class="card-body">
    ...
    <!-- page end-->
@include('master.blog_management.blog_category.create')
@include('master.blog_management.blog_category.edit')
@include('master.blog_management.blog_category.trans')
...
@push('js')
    ...
    <script>
        var table = $('#datatable').DataTable({
            processing: true,
            serverSide: true,
            ajax: '{{ route('master.blog_category.index')}}',
            columns: [
                {data: 'id'},
                {data: 'category_title'},
                {data: 'img'},
