@extends('master.layout.index' ,['title' => _i('Edit Freelancer')])

@section('content')
    <!--breadcrumbs start -->
    <div class="page-header">
        <h4 class="page-title">{{ _i('Edit Freelancer') }}</h4>
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="{{ route('MasterHome') }}"><i class="fa fa-home"></i> {{ _i('Home') }}</a></li>
            <li class="breadcrumb-item active" aria-current="page">{{ _i('Edit Freelancer') }}</li>
        </ol>
    </div>
    <!--breadcrumbs end -->
            ...
            <div class="card">
                <div class="card-header">
                    <div class="card-title">
                        <h5> {{ _i('Edit Freelancer') }} </h5>
                    </div>
                </div>
                <div class="card-body">
                    <form method="post" action="{{ route('master.freelancers.update', $user->id) }}" class="form-horizontal" enctype="multipart/form-data"  data-parsley-validate="">
                        @csrf
                        @method('PATCH')
                    ...
                    <div style="display: inline" class="show_button_status">
                        @if($user->is_active == 1)
                            <form method="Post" action="{{route('master.admins.changeStatus')}}"
                                ...
                                <button type="submit" class="btn btn-primary  col-sm-3 inline ">
                                    {{_i('Deactivate')}}
                                </button>
                            </form>
                        @else
                ...
                success: function (response) {
                    if (response.is_active == 1) {
                        ...
                        $('.show_button_status').append('<form method="Post" action="{{route('master.admins.changeStatus')}}" class="status_button disaple_form" style="display: inline">' +
                            ...
                            '<button type="submit" class="btn btn-primary col-sm-3"> {{_i('Not Activate')}}</button></form>');
                    } else {
                        $('#user_status').empty();
                        $('#user_status').append(
                            '<div class="badge badge-warning btn btn-warning" > {{_i(" Not Active ")}} </div>');
