...
@extends('master.layout.index' ,['title' => _i('Commissions')])

@section('content')
    <!--breadcrumbs start -->
    <div class="page-header">
        <h4 class="page-title">{{ _i('Commissions') }}</h4>
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="{{ route('MasterHome') }}"><i class="fa fa-home"></i> {{ _i('Home') }}</a></li>
            <li class="breadcrumb-item active" aria-current="page">{{ _i('Commissions') }}</li>
        </ol>
    </div>
    <!--breadcrumbs end -->
                ...
                <div class="card-header">
                    <div class="card-title">
                        <a href="#" class='btn btn-shadow btn-primary mb-2 text-white' data-toggle='modal' data-target='#add-Modal'>
                            <i class="fa fa-plus"> </i> {{ _i('Create new commission') }}
                        </a>
                    </div>
                </div>
                <div class="card-body">
    ...
    <!-- page end-->
@include('master.commissions.create')
@include('master.commissions.edit')
@endsection
@push('js')
    <script>
        var table = $('#datatable').DataTable({
            processing: true,
            serverSide: true,
            ajax: '{{ route('master.commission.index')}}',
            columns: [
                {data: 'id'},
                {data: 'code'},
                {data: 'category_title'},
