        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="callout text-danger" id="masages_model_trans" style="display:none">
                    </div>
                    <form  action="{{ route('master.complaint_status.lang.store') }}" method="post" class="form-horizontal"  id="lang_submit" data-parsley-validate="">
                        @csrf
                        ...
                        <div class="box-body">
                            <div class="form-group ">
                                ...
                                <div class="col-sm-10">
                                    <input type="text" id="titletrans" class="form-control" name="title" required="" placeholder="{{_i('Please Enter Complaint Status Title')}}">
                                </div>
                                </div>
                            </div>
                        </div>
...
@push('js')
    <script>
        $('body').on('click','.lang_ex',function (e) {
            e.preventDefault();
            $('#masages_model_trans').empty();
            var id = $(this).data('id');
            var lang_id = $(this).data('lang');
            var lang_title = $(this).data('title');
            $.ajax({
                url: '{{ route('master.complaint_status.lang') }}',
                method: "get",
                data: {
                    lang_id: lang_id,
                    id: id,
                ...
                success: function (response) {
                    ...
                    if (response.errors){
                        ...
                        $.each(response.errors, function( index, value ) {
                            $('#masages_model_trans').show();
                            $('#masages_model_trans').append(value + "<br>");
                        });
                    }
                    if (response == 'SUCCESS'){
                        new Noty({
